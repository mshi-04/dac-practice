...
$destinationRoot = Join-Path $repositoryRoot '.codex/skills'
$excludedCodexSkillNames = @('atlas')
...
if (-not (Test-Path -LiteralPath $destinationRoot -PathType Container)) {
    if ($Check) {
        $differences.Add('Missing Codex skill directory: .codex/skills')
    } else {
...
if (Test-Path -LiteralPath $destinationRoot -PathType Container) {
    $generatedSkills = Get-ChildItem -LiteralPath $destinationRoot -Directory | Where-Object {
        $_.Name -notin $excludedCodexSkillNames
    }

    foreach ($generatedSkill in $generatedSkills) {
        if ($generatedSkill.Name -notin $sourceSkillNames) {
            $message = "Stale generated Codex skill: .codex/skills/$($generatedSkill.Name)"
            if ($Check) {
...
foreach ($sourceSkill in $sourceSkills) {
    ...
    if (-not (Test-Path -LiteralPath $destinationSkill -PathType Container)) {
        $differences.Add("Missing Codex skill: .codex/skills/$($sourceSkill.Name)")
        if ($Check) {
            continue
        }
    }
...
if ($Check) {
    ...
    Write-Output 'Codex skills are synchronized with .agents/skills.'
    exit 0
}

Write-Output 'Synchronized .agents/skills to .codex/skills.'
